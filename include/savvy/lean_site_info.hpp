...
namespace savvy
{
  ...
  class lean_site_info{
    ...
    public:
       ...
       */
      lean_site_info(int chrom, std::uint32_t pos, std::string ref, std::string alts,
        std::optional<std::string> id = std::nullopt,
        float qual = typed_value::missing_value<float>(), // std::numeric_limits<float>::quiet_NaN(), // bcf_missing_value = 0x7F800001
        std::vector<int> filters = {},
        std::vector<std::pair<std::int, typed_value>> info = {});
       ...
       */
      template<typename T>
      void set_info(decltype(info)::const_iterator off, const T& val)
      {
        (info.begin() + std::distance(info.cbegin(), off))->second = val;
      }
       ...
       */
      template<typename T>
      void set_info(const int& key, const T& val)
      {
        auto it = info.begin();
        for (; it != info.end(); ++it)
        {
